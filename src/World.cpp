...
vector<Filter*> World::get_filter() const {return filter_list;}
...
void World::simulate_sensor(default_random_engine &generator){
	cout << "Now simulating " << sensor_list.size() << " sensors..." << '\n';
...
// Add and simulate filters
void World::add_filter(Filter *f){
	filter_list.push_back(f);
	f->world = this;
}

void World::simulate_filter(default_random_engine &generator){
	cout << "Now simulating " << filter_list.size() << " filters..." << '\n';
	for (int TI=0; TI<num_times-1; ++TI){
		cout  << "TI = " << TI << endl;
