#include <iostream>
#include <cmath>
#include <random>

#include <Eigen/Dense>

#include "../include/Lie_group.h"
#include "../include/World.h"
#include "../include/Target.h"

using namespace std;
using namespace Eigen;


void Target::initialize(){
	state = MatrixXd::Zero(world->get_state_dim(), world->get_time());
	angular_velocity = MatrixXd::Zero(world->get_space_dim(), world->get_time());
	state.col(0) = IC; // Deterministic initialization
}


void Target::move(int TI, double dt, default_random_engine &generator){
	// state.col(TI+1) = move(state.col(TI), TI, dt);
	state.col(TI+1) = state.col(TI); // Target does not move for base class
}


void Sinusoidal::move(int TI, double dt, default_random_engine &generator){
	double t = TI*dt;
	angular_velocity(0,TI) = sin(2*M_PI*t/15.0);
	angular_velocity(1,TI) = -sin(2*M_PI*t/18.0 + M_PI/20.0);
	angular_velocity(2,TI) = cos(2*M_PI*t/17.0);

	// Add process noise in EACH velocity direction
	VectorXd process_noise = VectorXd::Constant(world->get_space_dim(),0);
	// default_random_engine generator;
	// generator.seed(static_cast<unsigned int>(time(0)));
	// srand(unsigned int) time((0));
	for (int d=0; d<angular_velocity.rows(); ++d){
		normal_distribution<double> distribution(0, process_noise_std(d)*sqrt(dt));
		process_noise(d) = distribution(generator);
	}

	state.col(TI+1) = dq(state.col(TI), angular_velocity.col(TI)*dt + process_noise); // Norm is 1
	// cout << "norm = " << state.col(TI+1).norm() << '\n';
	
}








